...
Supports FOUR CSV formats, auto-detected by column headers:
...
4. PROVISION FOR EXPENSES — Single-row, 1 debit + 1-2 credits (expense provision)
   Columns: Entry Type, Posting Date, Expense Account, Expense Amount,
            Supplier Name, Creditors Account, TDS Account, TDS Amount,
            Reference Number, Reference Date, Remarks, Mode of Payment
   Logic:
     - Debit line:  Expense Account for Expense Amount (no party)
     - Credit line 1: Creditors Account for (Expense Amount - TDS Amount)
                      with party_type=Supplier, party=Supplier Name
     - Credit line 2 (conditional): TDS Account for TDS Amount (no party)
       Only created if TDS Account and TDS Amount > 0
...
FORMAT_PROVISION = "provision_for_expenses"
...
PROVISION_MARKER_COLS = {"Expense Account", "Expense Amount", "Supplier Name", "Creditors Account"}
...
def detect_csv_format(headers):
    ...
    Returns FORMAT_SETTLEMENT, FORMAT_CUSTOMER_CREDIT, FORMAT_PROVISION, or FORMAT_GENERIC.
    ...
    if PROVISION_MARKER_COLS.issubset(header_set):
        return FORMAT_PROVISION
...
def load_csv(doc_name):
    ...
    Auto-detects format (Generic, Settlement, Customer Credit, or Provision)
    from headers.
    """
    ...
    elif csv_format == FORMAT_PROVISION:
        result = _load_csv_provision(rows, headers)
    else:
...
# ── CSV Loader: Provision for Expenses ────────────────────────────────────────

def _load_csv_provision(rows, headers):
    """
    Parse Provision for Expenses CSV — single-row format.
    Each row → 1 item + 2 or 3 accounting lines.

    Expense Account is debited for the full Expense Amount.
    Creditors Account is credited for (Expense Amount - TDS Amount)
        with party_type=Supplier, party=Supplier Name.
    TDS Account (optional) is credited for TDS Amount (no party).

    Balance: Expense Amount = Creditor Credit + TDS Amount.
    """
    required = ["Entry Type", "Expense Account", "Expense Amount",
                "Supplier Name", "Creditors Account"]
    missing = [h for h in required if h not in headers]
    if missing:
        frappe.throw(_("Missing required columns for Provision for Expenses format: {0}").format(
            ", ".join(missing)
        ))
    ...
    for csv_row_num, row in enumerate(rows, 2):
        ...
        expense_account = n.get("Expense Account", "")
        expense_amount = parse_amount(n.get("Expense Amount", ""))
        supplier_name = n.get("Supplier Name", "")
        creditors_account = n.get("Creditors Account", "")
        tds_account = n.get("TDS Account", "")
        tds_amount = parse_amount(n.get("TDS Amount", ""))

        # Creditor credit = Expense Amount - TDS Amount
        creditor_credit = expense_amount - tds_amount

        # Item (header) — no Slot Payment for provisions
        items.append({
            "je_group": group_idx,
            "entry_type": entry_type,
            "posting_date": parse_date(n.get("Posting Date", "")),
        ...
        # Line 1: Debit — expense account (no party)
        lines.append({
            "je_group": group_idx,
        ...
        # Line 2: Credit — creditors account with Supplier party
        lines.append({
            "je_group": group_idx,
            ...
            "party": supplier_name,
            "debit_amount": 0,
            "credit_amount": creditor_credit,
            "csv_row_number": csv_row_num,
        })

        # Line 3 (conditional): Credit — TDS account (no party)
    ...
    if group_idx == 0:
        frappe.throw(_("No valid Provision for Expenses entries found in CSV."))
...
def _detect_format_from_data(doc):
    ...
    Provision: every group has 2-3 lines, first line debit only (no party),
               second line credit with party_type=Supplier,
               optional third line credit (no party).
    Otherwise: Generic.
    """
    lines_by_group = {}
    for entry in doc.accounting_entries:
    ...
    for grp, group_lines in lines_by_group.items():
        ...
        if len(group_lines) not in (2, 3):
            is_provision = False
        else:
            # First line must be debit-only, no party
            if not (group_lines[0].debit_amount and not group_lines[0].credit_amount
                    and not group_lines[0].party_type):
                is_provision = False
            # Second line must be credit with Supplier party
            if not (group_lines[1].credit_amount and not group_lines[1].debit_amount
                    and group_lines[1].party_type == "Supplier"):
                is_provision = False
            # Third line (if present) must be credit, no party
            if len(group_lines) == 3:
                if not (group_lines[2].credit_amount and not group_lines[2].debit_amount
                        and not group_lines[2].party_type):
                    is_provision = False
...
def get_csv_template(template_type=None):
    ...
    template_type: 'generic' (default), 'settlement', 'customer_credit',
                   or 'provision_for_expenses'
    """
    ...
    elif template_type == "provision_for_expenses":
        return _get_provision_template()
    else:
...
def _get_provision_template():
    """Provision for Expenses single-row template.
    Expense Amount = Creditor Credit + TDS Amount (auto-balanced).
    TDS Account and TDS Amount are optional — leave blank for entries without TDS.

    Row 1: With TDS — Expense 4600000, TDS 92000, Creditor gets 4508000
    Row 2: Without TDS — Expense 150000, no TDS, Creditor gets full 150000
    """
    return (
        "Entry Type,Posting Date,Expense Account,Expense Amount,"
        "Supplier Name,Creditors Account,TDS Account,TDS Amount,"
        "Reference Number,Reference Date,Remarks,Mode of Payment\n"
        "Journal Entry,2025-03-01,Digital Marketing Agency Expenses - BDPL,4600000,"
        "Facebook,Creditors - BDPL,TDS on Contractors Payable - BDPL,92000,"
        "REF-001,2025-03-01,Facebook ads provision March,\n"
        "Journal Entry,2025-03-01,Office Maintenance Expenses - BDPL,150000,"
        "ABC Facility Services,Creditors - BDPL,,,,"
        ",Office maintenance provision March,\n"
    )
